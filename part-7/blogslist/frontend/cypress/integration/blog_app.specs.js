describe('Blog app', function () {
  ...
  describe('Login', function () {
    it('fails with wrong credentials', function () {
      ...
      cy.get('#error-message').should('have.css', 'border', '2px solid rgb(255, 0, 0)');
    });
  ...
  describe('When logged in', function () {
    ...
    it('A blog can be created', function () {
      ...
      cy.get('#success-message').should('have.css', 'border', '2px solid rgb(0, 128, 0)');
    });
  });
    ...
    describe('several blogs exist', function () {
      ...
      it.only('are sorted by likes', function () {
        ...
        cy.get('#blogs-list')
          ...
          .wait(500)
          .then(($c) => {
            const likes = $c.map((_, html) => Cypress.$(html).text().replace('like', '')).get();
